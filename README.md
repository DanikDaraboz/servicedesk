...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Docker-Sail-2496ED?style=flat&logo=docker" alt="Docker">
...
-   [x] **Docker + Laravel Sail** — полное окружение в контейнерах
-   [x] **Аутентификация и регистрация** — Laravel Breeze с Blade
-   [x] **CRUD заявок** — создание, просмотр, редактирование, удаление
-   [x] **Админ-панель** — управление всеми заявками, смена статусов
-   [x] **Ролевая модель** — пользователь (user) и администратор (admin)
-   [x] **UI/UX с Tailwind CSS** — адаптивный и современный интерфейс
-   [x] **База данных** — миграции, сидеры с тестовыми пользователями
-   [x] **CI/CD** — GitHub Actions workflow для автоматического тестирования
-   [x] **GitHub репозиторий** — полная история коммитов
...
1. Настроить правильные переменные окружения в `.env`
2. Использовать `APP_ENV=production`
3. Настроить настоящий SMTP сервер для отправки почты
4. Использовать `php artisan config:cache` и `php artisan route:cache`
5. Настроить Nginx/Apache для обслуживания статических файлов
